...
class StockPredictionEngine:
        ...
        if self.df_fund is not None:
            for _, row in self.df_fund.iterrows():
                ...
                if ticker:
                    ...
                    self.fundamental_cache[ticker] = {'cap': cap_val, 'pe': pe_val}
    ...
    def create_features(self, ticker, forecast_days):
        ...
        company_data['bb_width'] = (company_data['bb_upper'] - company_data['bb_lower']) / (company_data['bb_middle'] + 1e-6)
        company_data['bb_position'] = (company_data['price'] - company_data['bb_lower']) / (company_data['bb_upper'] - company_data['bb_lower'] + 1e-6)
    ...
    def train(self, forecast_days=5):
        try:
            ...
            y_direction_proba = self.models['direction'].predict_proba(X_test)[:, 1]

            r2 = r2_score(y_return_test, y_return_pred)
            acc = accuracy_score(y_direction_test, y_direction_pred)
            precision = precision_score(y_direction_test, y_direction_pred, zero_division=0)
            recall = recall_score(y_direction_test, y_direction_pred, zero_division=0)
            f1 = f1_score(y_direction_test, y_direction_pred, zero_division=0)
            roc_auc = roc_auc_score(y_direction_test, y_direction_proba)
...
class MainWin(QtWidgets.QMainWindow):
    ...
    def train_model(self):
        ...
        try:
            ...
            if metrics:
                self.result.setText(f"✅ Модель обучена! ROC-AUC = {metrics['roc_auc']:.4f}, F1 = {metrics['f1']:.4f}")
                msg = QtWidgets.QMessageBox()
                msg.setWindowTitle("Модель обучена")
                msg.setText(
                    f"✅ Модель прогнозирования успешно обучена!\n\n"
                    f"📊 ROC-AUC: {metrics['roc_auc']:.4f} (главная метрика)\n"
                    f"🎯 F1-score: {metrics['f1']:.4f}\n"
                    f"📈 R² (доходность): {metrics['r2']:.4f}\n"
                    f"📋 Размер выборки: {metrics['samples']} записей")
                msg.setIcon(QtWidgets.QMessageBox.Information)
                msg.exec_()
            else:
